...
import { isDevDemo, demoAdmin } from '@/config/demoAccounts';
...
function LoginForm() {
  ...
  const publicWebUrl = webUrl();

  useEffect(() => {
    if (params.get('denied') !== '1') return;
    void getSupabaseBrowser().auth.signOut();
    setError('Cet espace est réservé à l’équipe Taxi Na Biso.');
  }, [params]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!isStaffRole(role)) {
        ...
        throw new Error('Cet espace est réservé à l’équipe Taxi Na Biso.');
      }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#161b27] text-white">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        ...
        <svg className="tnb-marker-soft absolute left-[16%] bottom-[24%] h-8 w-8 text-[#F04A18] opacity-30" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-4 py-16 lg:grid-cols-[1fr_minmax(360px,440px)]">
        <div>
          ...
          <p className="mt-3 max-w-md text-sm text-white/70">
            Accès réservé à l’équipe Taxi Na Biso.
          </p>
        </div>
        ...
        >
          ...
          <p className="mt-4 text-center text-sm text-[#6b7280]">
            <a href={publicWebUrl} className="text-[#F04A18]">
